	...
	For ($passe; 0; 1)
		...
		While (Month of:C24($day)=($monthInDrawing))
			...
			Case of 
				...
				: (Form:C1466.current_item.medicalCapabilities=Null:C1517)
				Else 
					...
					If ($workingPeriods.workingRanges.length>0)
						...
						$duration:=Form:C1466.current_item.appointments.query("startStmp >= :1 & startStmp < :2"; $from; $to).sum("duration")
						If ($duration#0)
...
If ($calculateLB_appointment)
	...
	For each ($appt; ds:C1482.Appointment.query("UUID_MedicalStaff = :1 & startStmp >= :2 & startStmp < :3"; Form:C1466.current_item.UUID; $from; $to))
		If (Form:C1466.lb_appointments_toDelete.indexOf($appt.UUID)=-1)
			...
			$appointment.dataclass:="Appointment"
			$appointment.who:=Person_getFullName($appointment.UUID_Person)
			Form:C1466.lb_appointments.push($appointment)
		End if 
	End for each 
	For each ($appointment; Form:C1466.lb_appointments_toSave.query("UUID_MedicalStaff = :1 & startStmp >= :2 & startStmp < :3"; Form:C1466.current_item.UUID; $from; $to))
	...
	For each ($appointment; Form:C1466.lb_appointments)
		...
		$meta:=New object:C1471("cell"; New object:C1471("columnColor"; New object:C1471("fill"; $appointment.columnColor)))
		Case of 
			: ($appointment.idStatus=-111) | ($appointment.idStatus=-112)
...
If ($showDetail)
	Case of 
		...
		: (Form:C1466.current_appointment.dataclass="Appointment")
	...
	$indicesKind:=Form:C1466.workingPeriods.kinds.indices("UUID = :1"; String:C10(Form:C1466.current_appointment.UUID_ConsultationKind))
	...
	Form:C1466.lb_status:=Form:C1466.current_appointment.chronoStatus.histo.copy()
	For each ($status; Form:C1466.lb_status)
		$status.time:=sfw_Time(sfw_stmp_read_time($status.stmp))
		$status.date:=sfw_Time(sfw_stmp_read_date($status.stmp))
		$status.statusLabel:=sfw_xliff_read("chrono.status_"+String:C10($status.status); String:C10($status.status))
		$status.originLabel:=sfw_xliff_read("chrono.origin_"+String:C10($status.origin); String:C10($status.origin))
	End for each 
End if 
